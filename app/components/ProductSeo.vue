...
const baseImage: string =
	props.info.image?.sourceUrl || "/images/placeholder.jpg";
...
const absolutize = (u?: string) =>
	!u ? "" : u.startsWith("http") ? u : `${url.origin}${u}`;
const ogImage = absolutize(ogSrc);
const twitterImage = absolutize(twSrc);

const rawDescription =
	props.info.shortDescription && stripHtml(props.info.shortDescription)
		? stripHtml(props.info.shortDescription)
		: stripHtml(props.info.description);
const description = rawDescription?.slice(0, 160) || "";

const alternates = computed(() => {
	const items: Array<{ href: string; hreflang: string }> = [];
	const codes = (locales.value as any[]).map((l) => ({
		code: l.code,
		iso: l.iso,
	}));
