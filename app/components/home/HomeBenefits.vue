<!--app/components/home/HomeBenefits.vue-->
<script setup>
const benefits = [
...
<template>
	<section class="px-3 lg:px-5 pt-3 lg:pt-5 my-10">
		<div class="grid grid-cols-2 gap-3 lg:grid-cols-4 lg:gap-5">
			<div
				v-for="benefit in benefits"
				:key="benefit.key"
				class="flex flex-row items-center gap-3 rounded-[24px] bg-black/5 p-4 dark:bg-white/10">
				<div
					class="flex h-12 w-12 flex-none items-center justify-center rounded-full bg-primary-400/15 text-primary dark:text-primary-300">
					<UIcon
						:name="benefit.icon"
						size="26" />
				</div>
